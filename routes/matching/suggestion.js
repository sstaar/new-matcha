...
router.post('/suggestion', async (request, response) => {
    ...
    try {
        ...
        res = res.filter((item) => {
            ...
            return (distanceBetween < 10000 && (userOrientation === item.gender || userOrientation === 'both'))
        });
        res = res.sort((user1, user2) => {
            let distance1 = distance(info.latitude, info.longitude, user1.latitude, user1.longitude);
            let distance2 = distance(info.latitude, info.longitude, user2.latitude, user2.longitude);
        ...
        res.forEach((item) => {
            item.distance = distance(info.latitude, info.longitude, item.latitude, item.longitude);
        });
